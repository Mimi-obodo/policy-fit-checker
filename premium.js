...
(function () {
  ...
  /* ---------- intro overlay (first-load sequence) ---------- */
  (function initIntro() {
    ...
    try { alreadySeen = sessionStorage.getItem("pfc-intro-seen") === "1"; } catch (e) {}
    ...
    var MIN_MS = introReduced ? 800 : 1500;
    var MAX_MS = 3500;
    ...
    function reveal() {
      ...
      overlay.classList.add("intro-revealing");
      setTimeout(function () {
        overlay.classList.add("intro-hidden");
        try { sessionStorage.setItem("pfc-intro-seen", "1"); } catch (e) {}
      }, 650);
    }

    function tryReveal() {
      var elapsed = Date.now() - startTime;
      if (elapsed >= MIN_MS && fontsReady) reveal();
      else if (elapsed >= MAX_MS) reveal();
      else setTimeout(tryReveal, 80);
    }
    ...
    if (document.fonts && document.fonts.ready) {
      ...
      setTimeout(function () { fontsReady = true; }, 600);
    }

    /* Warm catalog (best-effort, non-blocking) */
    try {
      var warmCfg = (window.PFC_CONFIG || {}).WORKER_URL || "";
      if (warmCfg) {
        fetch(warmCfg + "/api/catalog", { method: "GET" })
          .catch(function () {});
      }
    } catch (e) {}
    ...
    if (introReduced) {
      overlay.classList.add("intro-wordmark-visible", "intro-tagline-visible", "intro-loader-visible");
    } else {
      setTimeout(function () {
        overlay.classList.add("intro-drawing");
        overlay.classList.add("intro-wordmark-visible");
      }, 200);
      setTimeout(function () {
        overlay.classList.add("intro-tagline-visible");
      }, 2000);
      setTimeout(function () {
        overlay.classList.add("intro-loader-visible");
      }, 2400);
    }

    /* Reveal when ready or timeout */
    setTimeout(tryReveal, MIN_MS);
  })();
